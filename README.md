...
| [WRITING.md](./docs/WRITING.md) | **금지어 38개**, 용어 확정표, 마이크로카피 전량, 인사팀 공지문 |
...
| [EDITOR.md](./docs/EDITOR.md) | BlockNote 커스텀 스키마, 어댑터, **IME 구현 + CDP 회귀 테스트 10건** |
...
**"안 하겠다"를 정책이 아니라 구조로 만들었다.** 반복해서 나타나는 패턴이다.

| 약속 | 구조적 강제 |
|---|---|
| 관리자는 개인 문서를 못 본다 | `resolveDocumentAccess()`가 `org_members`를 **인자로도 안 받는다** |
| 매칭은 원문을 보지 않는다 | `SocketFeature` 타입에 **원문 필드가 없다** |
| 비공개 노트는 안 나간다 | 렌더 트리에 있으면 **빌드 실패** |
| 조직 RAG로 검색 우회 불가 | **인덱스를 스키마에 안 만든다** |
| 이벤트에 값이 안 담긴다 | zod `.strict()` — **그 필드가 스키마에 없다** |

정책 문서는 잊히지만 **타입에 없는 필드는 6개월 뒤에도 없다.**
...
의견이 충돌한 지점과 해소 근거는 [DECISIONS.md](./docs/DECISIONS.md)에 있다.
