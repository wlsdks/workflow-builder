...
**강제 방법** — 이벤트 스키마를 **`packages/analytics-schema`**(별도 패키지 — `graph-core`의 `dependencies: {}`는 영구 계약이므로, D-119)에 zod `.strict()`로 정의 → 수집 프록시에서 unknown key 발견 시 **이벤트 drop + 알림.** 문자열은 원칙적으로 `enum` 또는 `*_band`만 허용. **자유 텍스트 속성은 스키마에 존재하지 않는다.**
...
**초안의 4주 연속 실행안은 M4를 전제하고 있었다.** W3는 체크리스트·핸드오프(M4 기능), W4는 신선도 프롬프트(**어느 마일스톤에도 없던 기능**)를 요구한다.

M1 직후 그대로 돌리면 게이트가 자동으로 미달하고 **"죽은 문서 생성기" 판정**이 나온다.
그런데 그건 **제품이 실패한 게 아니라 계측 설계가 만든 실패**다. 제품이 멀쩡한데 계측이 사형선고를 내리는 구조였다.

→ **가설이 다르면 파일럿도 다르다.** 두 개로 쪼갠다.

| | 시점 | 기간 | 검증하는 가설 | 필요 기능 |
|---|---|---|---|---|
| **파일럿 A** | M1.5 직후 | 2주 | **입력 절벽을 넘는가 · 남에게 보여줄 만한가** | 붙여넣기·질문연쇄·에디터·저장·공유·내보내기 |
| **파일럿 B** | M4a 직후 | 2주 | **문서가 살아 있는가** | + 체크리스트·신선도·핸드오프 |

**A와 B 사이에 최소 6주가 있어야 한다** — D14 재편집률은 정의상 14일이 지나야 측정된다. A 참가자를 그대로 B로 데려가면 그 사이 자연 사용 데이터가 B의 기준선이 된다.

---

## 파일럿 A (M1.5 직후 · 2주) — 입력과 공유

| 주 | 활동 | 담당 | 산출물 |
|---|---|---|---|
| **A1** | 킥오프 45분(약속 2개 + 금지선 낭독 5분) → **1인 1문서 워크숍 60분** (전원 동석, **제품팀은 관찰만·개입 금지**, 손 든 경우만 지원) | PM 진행 / 리서처 2인 관찰 | 참가자당 문서 1개, 관찰 노트, **개입 요청 횟수·사유 로그** |
| **A2** | **자연 사용 주** (공지 없음, 리마인드 없음). 병행: **1:1 사용성 테스트 45분 × 5명** → 회고 60분 | 리서처 | 사용성 이슈(심각도 3단), 두 번째 문서 작성률, A 판정 리포트 |

**게이트**
- **A1**: 세션 내 저장 완료 **≥80%**, 워크숍 보정 TTFD 중앙값 **≤12분**, 세션당 드래그 시도 중앙값 **<2**, 지원 개입 **≤1.5회/인**
- **A2**: **지원 없이 2번째 문서 작성 ≥50%**, 공유율 **≥30%**, 열람자/작성자 **≥2:1**, 심각도 1(작업 완수 불가) 이슈 **0건**

**A에서 측정하지 않는 것** — D14 재편집률, 신선도 응답률, 체크리스트 수정 건수.
기능이 없어서가 아니라 **아직 시간이 안 지났기 때문**이다. 이 셋을 A에서 재면 반드시 0이 나오고, 그 0은 아무 의미가 없다.

---

## 파일럿 B (M4a 직후 · 2주) — 생존

A 참가자를 **그대로** 데려간다. 그동안의 자연 사용이 B의 기준선이다.

| 주 | 활동 | 담당 | 산출물 |
|---|---|---|---|
| **B1** | **공유·전파 주.** 실제 채용 프로세스를 체크리스트 모드로 진행. 핸드오프 초대 실사용 | 인사팀 챔피언 / 제품팀 지원 | 체크리스트 수정 건수, 접합 확정 수, 초대 수락률 |
| **B2** | 신선도 슬랙 프롬프트 발송 → 회고 90분 + **비사용자·저사용자 인터뷰 4건** | 리서처 / PM | 인터뷰 전사, 판정 리포트, 2단계(계기 확산) 진행 여부 결정 |

**게이트**
- **B1**: 체크리스트 실행 문서당 수정 **≥1건**, 문서당 명시 소켓 **≥0.5** (미달 시 접합 조립을 M5로 — ASSEMBLY 킬 스위치)
- **B2**: 신선도 응답률 **≥40%**, **D14 재편집률 ≥25%**, "다른 부서에 추천하겠다" **≥7/10**

**A와 B 사이 (약 6주)** — 개입하지 않는다. 리마인드도 공지도 없다. **이 구간의 자연 이탈이 D14 재편집률의 진짜 값**이고, 여기에 손을 대면 B의 결과를 못 믿는다.
...
- **A2** 종료 시 지원 없이 두 번째 문서 작성 **<25%** → 입력 절벽 미해결. **제품 문제**
- **B2** D14 재편집률 **<10%** → **"죽은 문서 생성기" 판정.** M4(체크리스트·핸드오프) 완료 전 확산 금지
